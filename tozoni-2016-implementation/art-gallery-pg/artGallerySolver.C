...
int g_traceStepCounter = 0;

void resetTraceStepCounter() { g_traceStepCounter = 0; }

void writeRing(std::ostream& o, const Polygon& poly) {
    o << poly.size() << "\n";
    for (Vertex_iterator v = poly.vertices_begin(); v != poly.vertices_end(); ++v) {
        o << CGAL::to_double(v->x()) << " " << CGAL::to_double(v->y()) << "\n";
    }
}

/**
 * If AGSOL_TRACE_DIR is set, dump one text file per solveIteration with guards,
 * visibility polygons, and uncovered geometry (for offline visualization).
 */
void dumpTraceIfEnabled(
    int outerIter,
    int innerIter,
    RT areaMiss,
    const std::vector<Point>& guards,
    const std::vector<PolygonExt>& visGuards,
    const std::list<PolygonWithHoles>& notCovered) {
    const char* dir = std::getenv("AGSOL_TRACE_DIR");
    if (dir == NULL || std::strlen(dir) == 0) {
        return;
    }
    g_traceStepCounter++;
    std::ostringstream path;
    path << dir << "/step_" << std::setw(4) << std::setfill('0') << g_traceStepCounter << ".txt";
    std::ofstream f(path.str().c_str());
    if (!f) {
        return;
    }
    f << "AGSOL_TRACE_V1\n";
    f << "outer_iteration " << outerIter << "\n";
    f << "inner_iteration " << innerIter << "\n";
    f << "area_miss " << areaMiss.to_double() << "\n";
    f << "num_guards " << guards.size() << "\n";
    for (size_t i = 0; i < guards.size(); ++i) {
        f << "guard " << CGAL::to_double(guards[i].x()) << " " << CGAL::to_double(guards[i].y()) << "\n";
    }
    f << "num_visibility " << visGuards.size() << "\n";
    for (size_t i = 0; i < visGuards.size(); ++i) {
        f << "visibility_polygon " << i << "\n";
        writeRing(f, visGuards[i]);
    }
    f << "num_uncovered " << notCovered.size() << "\n";
    int u = 0;
    for (std::list<PolygonWithHoles>::const_iterator it = notCovered.begin(); it != notCovered.end(); ++it) {
        const PolygonWithHoles& pwh = *it;
        f << "uncovered " << u++ << "\n";
        writeRing(f, pwh.outer_boundary());
        f << "num_holes " << pwh.number_of_holes() << "\n";
        for (PolygonWithHoles::Hole_const_iterator hit = pwh.holes_begin(); hit != pwh.holes_end(); ++hit) {
            writeRing(f, *hit);
        }
    }
}
...
void ArtGallerySolver::insertDisPointsToArrange(vector<Point> dis){
    ...
    double t1;
    PolygonExt visPol;
    std::unordered_map<Point, PolygonExt, PointHash>::iterator it;
...
void ArtGallerySolver::selectGuardCandidates(){
    ...
    std::unordered_map<Point, int, PointHash> candidatesUsed; 
    std::unordered_map<Point, int, PointHash>::iterator itMap; 
...
void ArtGallerySolver::addIterationGridPoints() {
    ...
    std::unordered_map<pair<Point, Point>, bool, PointPointHash>::iterator itMap;
    std::unordered_map<Point, PolygonExt, PointHash>::iterator itVis;
    bool areVisible;
    ...
    if (_solverMode == 3) {
        /* Mode 3: bbox pre-filter */
        std::vector<CGAL::Bbox_2> candBB(nCols);
        for (int j = 0; j < nCols; j++) {
            candBB[j] = _guardCandidates.at(j).bbox();
        }
        ...
        for(int i = 0; i < nRows; i++) {
            ...
            CGAL::Bbox_2 visBB = visPolCache.bbox();

            for(int j = 0; j < nCols; j++) {
                pair<Point, Point> pointsPair(_guardCandidates.at(j), p);

                if (!CGAL::do_overlap(candBB[j], visBB)) {
                    areVisible = false;
                } else if (visPolCache.bounded_side(_guardCandidates.at(j)) == CGAL::ON_UNBOUNDED_SIDE) {
                    areVisible = false;
                } else {
                    areVisible = true;
                }
        ...
        /* Modes 1, 2, 4: original algorithm */
        for(int i = 0; i < nRows; i++) {
            Point p = grid.at(i);
            PolygonExt visPolCache = _polygon.getVisibility(p);
            _visCache.insert(pair<Point, PolygonExt>(p, visPolCache));
...
RT ArtGallerySolver::solveIteration() {
    ...
    dumpTraceIfEnabled(_mainLoopIterations, _iteration, areaMiss, _guards, _visGuards, notCovered);
...
void ArtGallerySolver::initSolver(PolygonWithHolesExt pol, vector<Point> guardCandidates) {
    ...
    if (_solverMode == 3) {
        /* Mode 3: bbox pre-filter with witness-outer loop order */
        std::vector<CGAL::Bbox_2> candBB(nCols);
        for (int j = 0; j < nCols; j++) {
            candBB[j] = _guardCandidates.at(j).bbox();
        }
        ...
        if (_mainLoopIterations == 1) {
            for (int i = 0; i < nRows; i++) {
                ...
                const PolygonExt& visPolRef = itVis->second;
                CGAL::Bbox_2 visBB = visPolRef.bbox();
                ...
                for (int j = 0; j < nCols; j++) {
                    ...
                    if (!CGAL::do_overlap(candBB[j], visBB)) {
                        ...
                        areVisible = (visPolRef.bounded_side(pVis) != CGAL::ON_UNBOUNDED_SIDE);
                    }

                    _visibilityTest.insert(pair<pair<Point, Point>, bool>(pointsPair, areVisible));
                    _matrix[i][j] = areVisible;
                }
            }
        }
        else {
